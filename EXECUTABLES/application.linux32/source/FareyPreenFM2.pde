...
switch(key) { 
 ...
 case 'r': // randomize all  
    fareyorder = 1+int(random(15));
    fareyidx = int(random(5));
    c1idx = int(random(15));
    c2idx = int(random(15));    
    recompute();
    break;        
...
void computefamratios(int n, int c, int m)
{
...
void printfamratios(int n, int c, int m)
{
  clear();
...
void    send13pair(int car, int mod)
{  
...
void    send24pair(int car, int mod)
{  
    int carlsb,carmsb,modlsb,modmsb;
...
void konfig() // full configuration change to set up preen to have alg 4, sin waves, mix,etc
{  
